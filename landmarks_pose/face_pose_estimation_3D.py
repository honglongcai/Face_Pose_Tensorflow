...
f=open('fail_detect_3D','w')
face_file=open('pose_estimation_3D.csv','w')
...
startT=time.time()
directories=glob.glob('lfw_test/*')
directories.sort()
for directory in directories:
	...
	for image in images:
		...
		dic=[{'Path':image,'Yaw':htheta,'Pitch':vtheta,'Roll':rtheta,'v0':A[0,0],'v1':A[0,1],'v2':A[0,2],'v3':A[1,0],'v4':A[1,1],'v5':A[1,2],'v6':A[2,0],'v7':A[2,1],'v8':A[2,2],'FN':nf}]
		f_csv.writerows(dic)
		d=np.arange(5)
		fig=plt.figure()
		my_dpi=fig.get_dpi()
		...
		ax.view_init(0,0)
		ax.plot(A[2,0]*d,A[0,0]*d,A[1,0]*d)
		ax.plot(A[2,1]*d,A[0,1]*d,A[1,1]*d)
		ax.plot(A[2,2]*d,A[0,2]*d,A[1,2]*d)
		ax.set_xlabel('X')
		ax.set_ylabel('Y')
		ax.set_zlabel('Z')
		ax.set_title(strh+', '+strv+', '+strr)
		pat1='pose3D/'+imgpath[2]
